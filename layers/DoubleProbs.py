...
class DoublePCell(LayerRNNCell):
    def __init__(self,
                 num_units,
                 sent_cell: RNNCell,
                 r_cell: RNNCell,
        ...
        super(DoublePCell, self).__init__(_reuse=reuse, name=name, dtype=dtype, **kwargs)
        ...
        if sent1_length is None:
            sent1_length = sent2_length = self.sent1.get_shape().as_list()[1]
    ...
    def pro_compute(self, rh):
        ...
        vec = tf.layers.dense(rh, units=self.num_units, activation=tf.tanh, name="aggregate")
        output = tf.layers.dense(vec, units=1, activation=tf.sigmoid, name="probs")
