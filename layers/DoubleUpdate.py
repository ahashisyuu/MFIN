...
class DoubleCell(LayerRNNCell):
    ...
    def build(self, inputs_shape):
        ...
        self.matrix_kernel = self.add_variable(name="matrix_kernel",
                                               shape=[1 * self.dim + self.num_units, 1],
                                               initializer=self.initializer)

        # self.fuse_kernel = self.add_variable(name="fuse_kernel",
        #                                      shape=[2 * self.dim, self.dim],
        #                                      initializer=self.initializer)
        # self.fuse_kernel1 = self.fuse_kernel
        # self.fuse_kernel2 = self.fuse_kernel
        ...
        if self.use_bias:
            ...
            #                                    shape=[self.dim],
            #                                    initializer=tf.constant_initializer(bias_init, dtype=tf.float32))
            # self.fuse_bias1 = self.fuse_bias
            # self.fuse_bias2 = self.fuse_bias
    ...
    def update_sentence(self,
        ...
        infor_cat = tf.concat([s1_tile - s2_tile, rh_tm_tile], axis=-1)   # (B, L1, L2, 1*dim + rdim)
        ...
        if s1_mask is not None:
            ...
            res_matrix1 = res_matrix - (1 - s1_mask_exp) * 10000.0
            res_matrix2 = res_matrix - (1 - s2_mask_exp) * 10000.0
        else:
            res_matrix1 = res_matrix
            res_matrix2 = res_matrix

        # adding TOP K
        m_s2 = self.top_k_att(res_matrix1, s1_tile, k=50, transpose=True)
        m_s1 = self.top_k_att(res_matrix2, s2_tile, k=50, transpose=False)
        ...
        # <2>[]
        m_s1 = layer_norm(m_s1 + s1)
        m_s2 = layer_norm(m_s2 + s2)
        ...
        # <3> UPDATE
        # s1_vec = tf.reduce_max(m_s1, axis=1)  # row-wise Max Pooling
        # s2_vec = tf.reduce_max(m_s2, axis=1)

        s1_vec = tf.reduce_sum(m_s1, axis=1) / tf.reduce_sum(self.sent1_mask, axis=1, keepdims=True)
        s2_vec = tf.reduce_sum(m_s2, axis=1) / tf.reduce_sum(self.sent2_mask, axis=1, keepdims=True)

        r_cell_input = tf.concat([s1_vec, s2_vec], axis=1)
        rh, _ = self.r_cell(r_cell_input, rh_tm)
