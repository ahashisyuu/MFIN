...
_OneStateTuple = collections.namedtuple("OneStateTuple", ("s", "r_h"))


class OneStateTuple(_OneStateTuple):
    __slots__ = ()
...
class OneCell(LayerRNNCell):
    def __init__(self,
                 ...
                 use_bias=False,
                 reuse=None, name="One", dtype=None, **kwargs):
        super(OneCell, self).__init__(_reuse=reuse, name=name, dtype=dtype, **kwargs)
        ...
        self.all_sent_mask = all_sent_mask
        ...
        self._state_size = OneStateTuple(self.sent_length * self.dim,  # sentences
                                         self.dim   # relation hidden states
                                         )
    ...
    def zero_state(self, batch_size, dtype):
        ...
        with tf.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            all_sent = tf.reshape(self.all_sent, [-1, self.sent_length * self.dim])

            rh = self.mark0

            state_list = [all_sent, rh[0]]

            return OneStateTuple(*state_list)
